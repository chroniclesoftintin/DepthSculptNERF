...
OUTPUT_TRAIN = 'train'
OUTPUT_TEST = 'test'
CAMERA_NAME = 'DepthSculptNERF Camera'


# blender nerf operator parent class
class DepthSculptNERF_Operator(bpy.types.Operator):
    ...
    def asserts(self, scene, method='SOF'):
        ...
        if method == 'COS' and CAMERA_NAME in scene.objects.keys():
            ...
            if not sphere_camera.data.type == 'PERSP':
                error_messages.append('DepthSculptNERF Camera must remain a perspective camera!')
        ...
        if method == 'COS' and any(x == 0 for x in scene.sphere_scale):
            error_messages.append('The DepthSculptNERF Sphere cannot be flat! Change its scale to be non zero in all axes.')
    ...
    def save_log_file(self, scene, directory, method='SOF'):
        ...
        logdata = {
            'DepthSculptNERF Version': scene.DepthSculptNERF_version,
